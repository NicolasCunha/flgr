...
| Kafka & HTTP Webhook Notification Delivery | Architecture | Defines how channels are actually delivered (async, with retries), and the HTTP signing secret referenced in the Data Model below. | [0009-kafka-and-webhook-notification-delivery.md](../../architecture/adr/0009-kafka-and-webhook-notification-delivery.md) |
...
Each HTTP channel has its own signing secret, generated when the channel is created. Every delivery to that channel includes an HMAC signature of the request body (computed with that secret), so the receiving system can verify the notification actually came from flgr — see [ADR-0009](../../architecture/adr/0009-kafka-and-webhook-notification-delivery.md) for the delivery mechanics (asynchronous, with retries) and the exact signing scheme.
...
| signing_secret_encrypted | BLOB | Yes | HTTP channels only: the HMAC signing secret, encrypted at rest per [ADR-0010](../../architecture/adr/0010-application-configuration-and-secrets.md); `NULL` for Kafka channels |
...
- [ ] Every HTTP channel delivery includes an HMAC signature of the request body, computed with that channel's own signing secret.
...
| Nicolas Filipe Cunha | Founder | 2026-08-16 | Added the `signing_secret_encrypted` column and HMAC signing details, to match ADR-0009 (written after this requirement, during implementation). |
